...
if [[ "$PLATFORM" == "mac" && "$PROFILE" == "work" ]]; then
    ...
    echo "  4. Sign into the Mac App Store (Drafts and Amphetamine install via mas)"
    echo "  5. Sign into Raycast with a work account, not your personal sync"
    echo "  6. Activate licenses: Keyboard Maestro, TextExpander, Hazel, Tower, Transmit"
    echo "  7. Import iTerm2 profile from backup"
    echo "  8. npm install -g @anthropic-ai/claude-code"
    echo "  9. Restart terminal and verify: source ~/.zshrc"
    echo ""
    echo "Not installed in work mode: most Mac App Store apps, Setapp and other"
    echo "personal-only licensed software, and Docker Desktop."
